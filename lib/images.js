...
// 빌드된 HTML의 <img>를 후처리해 WebP로 변환하고 표시 크기에 맞는 폭만 생성한다.
...
async function imageSrcset(src, widths) {
  ...
  return metadata.webp.map((file) => `${file.url} ${file.width}w`).join(", ");
}

// CSS background-image처럼 srcset을 쓸 수 없는 자리에서 단일 URL을 얻는다.
async function imageUrl(src, width) {
  const srcset = await imageSrcset(src, [width]);
