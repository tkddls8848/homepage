...
수동으로 복사해도 되지만, 스크립트를 쓰면 합친 뒤에 누락된 파일까지 알려줍니다.

```bash
# 기존 사이트의 images 폴더 두 개를 넘깁니다 (경로는 실제 위치로)
npm run import:images -- ./old-site/images ./old-site/product/images

# 인수 없이 실행하면 현재 무엇이 있고 없는지만 검사합니다
...
```
템플릿이 참조하는 이미지 12개 중 12개 확인됨.
✓ 참조된 이미지가 모두 준비되었습니다.
```

파일명이 원본과 다르면 누락 목록에 표시되므로, `src/_data/catalog.js` 의
`image` 값을 실제 파일명으로 고치거나 파일명을 맞춰 주세요.
